...
- **Aircraft photos DO work from the Cloud VM**: egress is domain-allowlisted, and `api.planespotters.net` / `t.plnspttrs.net` are reachable (unlike the flight-position APIs above). The mock fleet uses **real registrations**, so the web "Aircraft" card shows real photos in the demo. planespotters rejects requests whose `User-Agent` lacks a contact URL/email (HTTP 403) — keep the contact UA in `photos.DEFAULT_USER_AGENT` / `display.web.photo_user_agent`. If photos are ever unavailable the UI falls back to an SVG silhouette, so this never blocks.
